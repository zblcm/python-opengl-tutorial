...
class GLFWWindow:
    def __init__(self, path_smplx_model):
        ...
        self.device = "cuda"
        self.smplx_model = get_smplx_model(path_smplx_model, self.device)

        self.vao = glGenVertexArrays(1) # pylint: disable=W0612
    ...
    def run(self, path_npz, path_output):
        device = self.device
        ...
        while frame_index < frame_count:
            ...
            verts, norms = get_smplx_data(npz, self.smplx_model, frame_index, device)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        glfwWindow = GLFWWindow(R"D:\WorkingDirectory\FullBodyExternalData\body_models")
        for path_file_npz in sys.argv[1:]:
            if path_file_npz.endswith(".npz"):
                glfwWindow.run(
                    path_file_npz,
                    path_file_npz[:-4] + ".mp4",
                )
